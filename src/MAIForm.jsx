...
const dimensionMaxScores = {
  "Knowledge about Cognition": 8,
  "Procedural Knowledge": 4,
  "Conditional Knowledge": 4,
  "Planning": 6,
  "Information Management": 7,
  "Comprehension Monitoring": 6,
  "Debugging": 5,
  "Evaluation": 11,
};
...
/**
 * 根据原始得分数组计算标准化得分。
 * 标准化得分 = (原始得分 / 最大得分) * 10，保留一位小数。
 * @param {Array<{label: string, score: number}>} rawScores
 * @returns {Array<{label: string, standardized: number}>}
 */
function getStandardizedScores(rawScores) {
  return rawScores.map(({ label, score }) => {
    const max = dimensionMaxScores[label] ?? 0;
    let standardized = 0;
    if (max > 0) {
      standardized = Math.round(((score / max) * 10) * 10) / 10;
    }
